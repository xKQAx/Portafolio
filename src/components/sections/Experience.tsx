    ...
    <Section id="experience" title={t('title')}>
      <ul className="list-none m-0 p-0 relative">
        {EXPERIENCE_KEYS.map((key, idx) => (
          <li
            key={key}
            className={`relative pl-8 sm:pl-10 pb-8 sm:pb-10 ${idx === EXPERIENCE_KEYS.length - 1 ? 'pb-0' : ''}`}
          >
            <span
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap gap-2 sm:gap-3 mb-0.5 text-xs sm:text-sm text-slate-500">
                <span>{t(`${key}.period`)}</span>
                <span>{t(`${key}.location`)}</span>
              </div>
              <h3 className="text-base sm:text-lg font-semibold text-slate-100 mb-0.5">
                {t(`${key}.role`)}
              </h3>
              <p className="text-blue-400 text-sm mb-2">{t(`${key}.company`)}</p>
              <ul className="m-0 pl-4 sm:pl-5 text-slate-400 text-sm sm:text-[0.95rem] space-y-1">
